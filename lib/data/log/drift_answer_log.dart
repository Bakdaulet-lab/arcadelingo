...
class DriftAnswerLog implements AnswerLog {
  ...
  ///
  /// Убрать второй ключ мутацией и увидеть красноту нельзя — проверено: наш
  /// sqlite и без него отдаёт строки в порядке rowid, то есть в порядке
  /// вставки. Ключ и стоит ровно за тем, чтобы не зависеть от этого: SQL
  /// такого порядка не обещает, а сборок sqlite под нами три. Мутация из
  /// списка убрана как театр, сам ключ остаётся — разбор в `context.md`.
