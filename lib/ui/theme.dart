...
///
/// Живёт в `lib/ui/`, а не в `lib/app/`, с Фазы 2. Причина — правило
/// арх-гейта «фича не импортирует композиционный корень»:
/// `falling_words_views.dart` берёт отсюда `withWeight`, и из `lib/app/`
/// это был бы импорт корня из фичи. Тема — общая презентационная вещь, а не
/// корень, и место ей в слое представления.
///
