...
void main() {
  ...
  group('Переход серии', () {
    ...
    test('день раньше последнего — такой же разрыв, как пропуск', () {
      var state = advanceStreak(StreakState.empty, StreakDay(2026, 8, 25));
      state = advanceStreak(state, StreakDay(2026, 8, 26));
      ...
      expect(
        ...
        1,
        reason:
            'часы, переведённые назад, и перелёт на запад обрывают серию так '
            'же, как пропуск. Это решение Фазы 2; мягче с этим случаем должна '
            'обходиться Фаза 3 вместе с заморозкой и часовым поясом',
      );
      expect(backwards.best, 2);
    });
