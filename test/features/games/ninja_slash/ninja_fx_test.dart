...
void main() {
  ...
  group('Прилёт очков', () {
    ...
    // Мутация «u = phase» проходила мимо: страж до 35% на месте, а после
    // него доля пути прыгала сразу к easeIn(0.36). Непрерывность на старте
    // полёта — свойство, у которого не было меры.
    test('после задержки трогается с нуля, а не прыжком', () {
      // 0.01, а не 1e-3: easeInCubic — кубическая Безье, у неё на старте
      // не t³, а пологий, но не нулевой ход. У мутанта здесь ≈ 0.06.
      expect(scorePopFlight(0.36), lessThan(0.01));
      expect(scorePopFlight(0.36), greaterThan(0));
    });
